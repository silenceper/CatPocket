...
/*充值开始----------------------------------------------------------充值开始*/
	...
	*/	
    ...
    function card_edit()
    {
		...
		$index=$this->my_card_mod->get(array(
				'conditions' => "id='".$id."'",//条件 如：where
				'count' => true,
			));	
        $this->show_message($index['card_pass']);
        return;
    }	
    //删除充值卡
    function card_del()
	...
	function jifens_drop()//积分物品编辑 ORDER BY `ecm_my_jifen`.`id` ASC 
    {
		//删除积分物品
		$id = isset($_GET['id']) ? trim($_GET['id']) : '';
        if (!$id)
        {
            $this->show_warning('物品不存在！');
            return;
        }
        ...
        $this->show_message('删除积分兑换物品成功！');
        return;
	}
	
	
	function jifens_edit()//积分物品编辑 ORDER BY `ecm_my_jifen`.`id` ASC 
    {
	   if(!$_POST)
	   {
		   	$id = isset($_GET['id']) ? trim($_GET['id']) : '';
			...
			$index = $this->my_jifen_mod->get($id);
			$this->assign('val', $index);
		   //获取id查询并赋值
		    $this->display('jifen_add.html'); 
	   }
	   else
	   {
		   ...
		   $this->show_message('物品编辑置成功!','返回列表','index.php?module=my_money&act=jifen_shezhi');
		   return;
	   }
	}
