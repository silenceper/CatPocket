...
class StoreApp extends BackendApp
{
    ...
    function edit()
    {
		...
		$this->assign('taobao_rank', $taobao_rank);
		$this->assign('paipai_rank', $paipai_rank);
        if (!IS_POST)
        {
            /* 是否存在 */
        ...
        else
        {
            ...
            $data = array(
				...
				'taobao_rank'  => isset($_POST['taobao_rank'])? $_POST['taobao_rank']:$store['taobao_rank'],
				'paipai_rank'  => isset($_POST['paipai_rank'])? $_POST['paipai_rank']:$store['paipai_rank'],
                'sgrade'       => $_POST['sgrade'],
                'end_time'     => empty($_POST['end_time']) ? 0 : gmstr2time(trim($_POST['end_time'])),
                'state'        => $_POST['state'],
                'sort_order'   => $_POST['sort_order'],
        ...
        else
        {
            ...
            if (isset($_POST['agree']))
            {
                $this->_store_mod->edit($id, array(
                    'state'      => STORE_OPEN,
                    'add_time'   => gmtime(),
					'taobao_rank'  => isset($_POST['taobao_rank'])? $_POST['taobao_rank']:0,
					'paipai_rank'  => isset($_POST['paipai_rank'])? $_POST['paipai_rank']:0,
                    'sort_order' => 65535,
                ));
