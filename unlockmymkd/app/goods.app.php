...
class GoodsApp extends BackendApp
{
   ...
   function ajax_col()
   {
    ...
    function unchecked()
    {
	    $money = empty($_GET['money']) ? 0 : intval($_GET['money']);
	    if(!$money)
	    {
        ...
        $cate_mod =& bm('gcategory', array('_store_id' => 0));
        $this->assign('gcategories', $cate_mod->get_options(0, true));
	    $this->display('goods.unchecked.html');
    }
	...
	function check()
	{
		...
		$page = $this->_get_page();
		$goods_lists = $this->_goods_mod->get_list(array(
		...
		$goods_list = array();
		foreach ($goods_lists as $key => $goods)
		{
			$goods_list = $goods;
			$goods_list['cate_name'] = $this->_goods_mod->format_cate_name($goods['cate_name']);
		}
		$this->assign('goods_list', $goods_list);
		...
		{
			$is_true = isset($_POST['is_true'])?intval($_POST['is_true']):0;
			$ms =& ms();
			if($is_true==1)
			{
				...
				$content = '您的商品 "'.$goods_list['goods_name'].'" 审核成功，已上架';
				$ms->pm->send(MSG_SYSTEM, $goods_list['store_id'], '', $content);
				$this->show_message('审核成功，商品已上架！','back','index.php?app=goods');
				return;
			}
			else if($is_true==2)
			{
				...
				$content = '您的商品 "'.$goods_list['goods_name'].'" 审核失败，请修改商品参数，重新提交审核';
				$ms->pm->send(MSG_SYSTEM, $goods_list['store_id'], '', $content);
				$this->show_message('审核失败，商品已被禁售！','back','index.php?app=goods');
				return;
			}
		}
	}
