    ...
    function cancel_order()
    {
        ...
        else
        {
            ...
            foreach ($ids as $val)
            {
				...
				if($order_integral)
				{
					$integral = $integral_mod->get('user_id='.$order_info[$id]['seller_id']);// 当前是卖家
					$integral['jifen'] = $integral['jifen'] + $order_integral['buyer_has_integral'];
					
					$ids =  $integral_mod->get('user_id = '.$order_info[$id]['seller_id']);
					$idss =  $integral_mod->get('user_id = '.$order_info[$id]['buyer_id']);
					$id = $ids['id'];
					
					$integral_mod->edit($id,$integral);
					
					$integral = $integral_mod->get('user_id='.$order_info[$id]['buyer_id']);
					
					$integral['jifen'] = $integral['jifen'] + $order_integral['seller_has_integral'];
