...
class My_goodsApp extends StoreadminbaseApp
{
   ...
   function ajax_col()
   {
    ...
    function _check_post_data($data, $id = 0)
    {
		...
		/* 如果开启积分功能，则： 检查店家是否拥有足够的积分  by tyioocom */
		if(Conf::get('integral_enabled'))
		{
			if((!empty($_POST['has_integral']) && intval($_POST['has_integral'])<0) || (!empty($_POST['max_exchange']) && intval($_POST['max_exchange'])<0))
			{
				$this->_error('积分值必须大于或等于0');
				return false;
			}
	...
	function _get_owner_total_integral($id)
	{
		...
		$total_integral += $integral['amount'];
