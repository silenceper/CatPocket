...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        if (!IS_POST)
        {
			...
			$this->assign('yz_integral',$this->yz_integral($goods_info)); // by tyioocom 
        ...
        else
        {
			...
			if(Conf::get('integral_enabled'))
			{
			    ...
			    /*扣减操作*/
				$integral_mod = &m('my_money');
			    $integral = $integral_mod->get('user_id='.$this->visitor->get('user_id'));// 对买家
			    $amount = $integral['jifen']-intval($_POST['exchange_integral']); // 对买家，减去抵价的积分，获得的积分到交易完成后再加上去
			    $integral['jifen'] = $amount;
				
				$ids =  $integral_mod->get('user_id = '.$this->visitor->get('user_id'));
				$idss =  $integral_mod->get('user_id = '.$order_info['seller_id']);
				$id = $ids['id'];
				
				$integral_mod->edit($id,$integral);
				
				$integral = $integral_mod->get('user_id='.$order_info['seller_id']);
	...
	function yz_integral($goods_info)
	{
		...
		$data['max_exchange'] = $max_exchange > 0 ? $max_exchange : 0;
