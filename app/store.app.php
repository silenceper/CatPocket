...
class StoreApp extends StorebaseApp
{
    function index()
    {
		...
		$goods_integral = $goods_integral_mod->find(array(
			'fields'    => 'this.*',
			'join'      => '',
			'conditions'=> '',
		));
		foreach($goods_integral as $item)
		{
			$data=array(
			   'id' => $item['goods_id'],
			   'count' =>  $this->_get_sales_log($item['goods_id']),
			);
		}
		...
		$recom_goods=$this->_get_recommended_goods($id);
		
		foreach ($recom_goods as $key => $goods)
		{
			if(strlen($goods['goods_name'])>60)
			{
				$recom_goods[$key]['goods_name']=$this->msubstr($goods['goods_name'],0,60)."...";
			}
		
		}
        $this->assign('recommended_goods',$recom_goods);
		
		
		$new_groupbuy = $this->_get_new_groupbuy($id);
		foreach ($new_groupbuy as $key => $goods)
		{
			if(strlen($goods['goods_name'])>60)
			{
				$new_groupbuy[$key]['goods_name']=$this->msubstr($goods['goods_name'],0,60)."...";
			}
		
		}
		
        $this->assign('new_groupbuy',$new_groupbuy);
		
		
		$new_goods = $this->_get_new_goods($id) ;
		
		foreach ($new_goods as $key => $goods)
		{
			if(strlen($goods['goods_name'])>60)
			{
				$new_goods[$key]['goods_name']=$this->msubstr($goods['goods_name'],0,60)."...";
			}
	...
	/* 取得销售记录 */
    function _get_sales_log($goods_id)
    {
        $data = array();
    ...
    function _get_recommended_goods($id, $num = 12)
    {
        ...
        $goods_list = $goods_mod->find(array(
            ...
            'fields'     => 'this.*,goods_integral.*,goodsstatistics.*',
            'limit'      => $num,
			'join'  =>  'has_goodsintegral,has_goodsstatistics'
        ));
    ...
    function _assign_searched_goods($id)
    {
        ...
        foreach ($goods_list as $key => $goods)
        {
			...
			if(strlen($goods['goods_name'])>60)
			{
				$goods_list[$key]['goods_name']=$this->msubstr($goods['goods_name'],0,60)."...";
			}
