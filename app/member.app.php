...
class MemberApp extends MemberbaseApp
{
    ...
    function index()
    {
		...
		$this->assign('has_store',$user['has_store']);
        /* 店铺信用和好评率 */
        if ($user['has_store'])
        {
            $store_mod =& m('store');
			...
			$my_moneylog_list = $my_moneylog_mod->find(array(
				'fields'    => 'this.*',
				'conditions'=> "log_text='出售商品，资金已入账' and seller_id=".$user['user_id'],
				'order'     => 'id DESC',
				'limit'     => $page['limit'],
				'count'     => true
			));
    ...
    function register()
    {
        ...
        else
        {
			...
			// 如果开启了积分功能，则给新会员赠送积分  by tyioocom
			if(Conf::get('integral_enabled'))
			{
				$my_money_mod =& m('my_money');
				$ids =  $my_money_mod->get('user_id = '.$user_id);
				$id = $ids['id'];
				$data = array(
					'jifen' => Conf::get('register_integral'),
				);
				$my_money_mod->edit($id,$data);
			}
