...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function batch_edit()
    {
        if (!IS_POST)
        {
			 ...
			 $this->assign('integral_open',Conf::get('integral_enabled')); // by tyioocom 判断系统是否开启积分功能
        ...
        else
        {
			 ...
			 /*如果商品开启了积分功能，则可以批量编辑积分最多抵扣额度 by tyioocom */
   ...
   function ajax_col()
   {
       ...
       $data      = array('goods' => array(),
                          'specs' => array(),
                          'cates' => array());
       if (in_array($column ,array('goods_name','description', 'cate_id','show_index', 'cate_name', 'brand', 'spec_qty','if_show','closed','recommended')))
       {
    ...
    function _addible()
    {
        ...
        $payments = $payment_mod->get_enabled();
    ...
    function _get_goods_info($id = 0)
    {
		...
		/* 如果开启积分功能，则：读取商品积分设置 add by tyioocom */
		if(Conf::get('integral_enabled'))
		{
			$goods_integral_mod =& m('goods_integral');
    ...
    function _get_post_data($id = 0)
    {
		...
		$cate_info = $this->cate_mod->get($_POST['cate_id']);
		if($cate_info['is_checked']==1||$_POST['bargin_price']<Conf::get('min_price'))
		{
			$goods['if_show']=0;
			$goods['closed']=0;
			$goods['is_checked']=0;
		...
		/* 如果开启积分功能，则：  增加积分设置 edit by tyioocom */
		if(Conf::get('integral_enabled'))
		{
			$a = $_POST['has_integral'] * 0.01;
			if($a < 0.05 || $a >0.6) $a=0;
			else 
			{
				if(intval($_POST['state'])==0)
					$a = $a * $_POST['bargin_price'] / Conf::get('exchange_rate');
				else
				{
					$a = $a * ($_POST['bargin_price'] + $_POST['max_exchange'] * Conf::get('exchange_rate')) / Conf::get('exchange_rate');
				}
			}
			$goods_integral = array(
		      'has_integral' => $a,
			  'percent'  =>  $_POST['has_integral'],
		      'max_exchange' => $_POST['max_exchange'],
			  'bargin_price'  =>  $_POST['bargin_price'],
			  'integral_state'  =>  intval($_POST['state']),
		    );
			return array('goods_integral'=>$goods_integral,'goods' => $goods, 'specs' => $specs, 'cates' => $cates, 'goods_file_id' => $goods_file_id, 'desc_file_id' => $desc_file_id);
		}
		else // 原来官方的写法
		{
    ...
    {
		if(strstr($data['goods']['description'],'data:image'))
		{
            $this->_error('image_description_error');
            return;
		}
		...
		if($data['goods_integral']['has_integral']==0)
		{
			$this->_error('has_integral_error');
            return false;
		}	
		
        return true;
    }
	/* 获取店家的总积分  by tyioocom */
	function _get_owner_total_integral($id)
	{
		...
		$total_integral += $integral['jifen']+$integral['seller_edu'];
    ...
    function _save_post_data($data, $id = 0)
    {
        ...
        if ($id > 0)
        {
            ...
            $goods_id = $id;
			if(Conf::get('integral_enabled')){$this->_save_goods_integral($goods_id,$data['goods_integral'],'edit');} // 如果开启积分功能，则：更新商品积分设置  by  tyioocom
        }
        else
        {
            // add
            $goods_id = $this->_goods_mod->add($data['goods']);
			if(Conf::get('integral_enabled')){$this->_save_goods_integral($goods_id,$data['goods_integral']);} //如果开启积分功能，则： 添加商品积分设置 by  tyioocom
            if (!$goods_id)
            {
                $this->_error($this->_goods_mod->get_error());
                return false;
