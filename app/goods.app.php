...
class GoodsApp extends StorebaseApp
{
    ...
    function index()
    {
        ...
        else
        {
			...
			{
				if(strlen($goods['goods_name'])>60)
				{
					$data[$key]['goods_name']=$this->msubstr($goods['goods_name'],0,60)."...";
					$data[$key]['g_all']=$goods['goods_name'];
				}
			}
		...
		$this->assign('integral_open',Conf::get('integral_enabled'));  // by  tyioocom 控制在商品显示页显示
    ...
    function _get_common_info($id)
    {
        ...
        if ($data === false)
        {
			...
			{
				if($goods['is_checked']==0)
				{
					$this->show_warning('goods_is_checking');
                	return false;
				}
				else if($goods['is_checked']==1)
				{
					$this->show_warning('goods_failure');
					return false;
				}
				else
				{
    ...
    function _assign_common_info($data)
    {
        /* 商品信息 */
        $goods = $data['goods'];
		/*如果开启积分功能，则：商品信息增加积分设置，即 购买该商品可获得多少积分 by  tyioocom */
		if(Conf::get('integral_enabled'))
		{
			...
			if ($goods_integral)
			{
				$goods['has_integral']=$goods_integral['has_integral'];
				$goods['bargin_price']=$goods_integral['bargin_price'];
				$goods['integral_state']=$goods_integral['integral_state'];
				$goods['max_exchange']=$goods_integral['max_exchange'];
			} 
			else
			{
				$goods['has_integral']=0;
				$goods['bargin_price']=0;
				$goods['integral_state']=0;
				$goods['max_exchange']=0;
			}
		}
		/* end tyioocom */
    ...
    function _get_goods_history($id, $num = 9)
    {
        ...
        if ($goods_ids)
        {
            ...
            foreach ($goods_ids as $goods_id)
            {
				...
				if(strlen($rows[$goods_id]['goods_name'])>54)
				{
					empty($rows[$goods_id]['goods_name_1']) && $rows[$goods_id]['goods_name_1']=$this->msubstr($rows[$goods_id]['goods_name'],0,54)."...";
				}
				else
				{
					empty($rows[$goods_id]['goods_name_1']) && $rows[$goods_id]['goods_name_1']=$rows[$goods_id]['goods_name'];
				}
    ...
    {
          $goods_list = array();
          ecm_setcookie('goodsBrowseHistory', join(',',$goods_list));
          $this->json_result(array_values($goods_list));
    }
	...
	function jubao()
	{
		...
		else
		{
			...
			$datas = array(
				...
				'jubao_cate' => $_POST['jubao_cate'],
				'jubao_sm' =>  $_POST['jubao_sm'],
				'addtime' => gmtime(),
				'state' => 0,
			);
			$this->_jubao_mod->add($datas);
			$this->show_message('举报成功！');
		}
