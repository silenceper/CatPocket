...
class CartApp extends MallbaseApp
{
    ...
    function add()
    {
		...
		$model_cart =& m('cart');
		$cart_info = $model_cart->find("user_id=" . $this->visitor->get('user_id'). " AND session_id='" . SESS_ID . "'");
		$integral_mod =& m('goods_integral');
		$integral = $integral_mod->get($spec_info['goods_id']);
		if($integral['integral_state']==1)
		{
			if($cart_info)
			{
				$this->json_error('此类商品不能和其他商品一起购买，请先完成其他订单后单独购买此商品');
				/* 商品不存在 */
				return;
			}
		}
		...
		{
			$goods_se = $integral_mod->get($val['goods_id']);
			if($goods_se['integral_state']==1)
			{
				$this->json_error('此类商品不能和其他商品一起购买，请先完成其他订单后单独购买此商品');
				/* 商品不存在 */
				return;
    ...
    function add_only()
    {
        ...
        /* 是否购物车内有商品 */
        $model_cart =& m('cart');
		$item_info  = $model_cart->get("user_id=" . $this->visitor->get('user_id'). " AND session_id='" . SESS_ID . "'");
        if (!empty($item_info))
        {
            $this->show_warning('在购买商品前请先完成前一个订单，或到购物车去删除订单');
            return;
        }
        ...
        $this->show_message('添加成功','请继续完成订单','index.php?app=order&goods=cart&store_id='.$store_id);
    }
    ...
    function update()
    {
		...
		$goods_integral=$goods_integral_mod->get('goods_id='.$spec_info['goods_id']);
		$goods_price = $goods_integral['bargin_price']*$quantity;

		if($goods_integral['integral_state']==1)
			$goods_jifen = $goods_integral['max_exchange']*$quantity;
		...
		$subtotal_cx   =   $quantity * $goods_integral['bargin_price'];

        /* 返回JSON结果 */
        $cart_status = $this->_get_cart_status();
        $this->json_result(array(
            'cart'      =>  $cart_status['status'],                     //返回总的购物车状态
            'subtotal'  =>  $subtotal,                         //原价小计
			'subtotal_cx'  =>  $subtotal_cx,                   //现价小计
    ...
    function _get_carts($store_id = 0)
    {
        ...
        {
			$goods_integral=$goods_integral_mod->get('goods_id='.$item['goods_id']);
			$item['bargin_price'] = $goods_integral['bargin_price'];
			
			if(strlen($item['goods_name'])>60)
			{
				empty($item['goods_name_1']) && $item['goods_name_1']=$this->msubstr($item['goods_name'],0,60)."...";
			}
			else
			{
				empty($item['goods_name_1']) && $item['goods_name_1']=$item['goods_name'];
			}
			
			if($goods_integral['integral_state']==1)
				$item['max_exchange'] = $goods_integral['max_exchange'];
			else $item['max_exchange']=0;
			/* 小计 */
            $item['subtotal']   = $item['price'] * $item['quantity'];
			
			$item['subtotal_cx']   = $goods_integral['bargin_price'] * $item['quantity'];
            ...
            $carts[$item['store_id']]['amount']     += $item['subtotal_cx'];   //各店铺的总金额
