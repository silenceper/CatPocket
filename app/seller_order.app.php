...
class Seller_orderApp extends StoreadminbaseApp
{
    ...
    function cancel_order()
    {
        ...
        else
        {
            ...
            foreach ($ids as $val)
            {
				...
				$orders = $model_order->get('order_id='.$id);
				// 订单取消后，归还各自积分
				$order_integral = $order_integral_mod->get($id);
				if($order_integral)
				{				
					$integral_buyer = $my_money_mod->get('user_id='.$orders['buyer_id']);
					$my_money_mod->edit($integral_buyer['id'],'jifen=jifen+'.$order_integral['seller_has_integral']);
		
					$integrallog_mod->add(array(
						'user_id' => $this->visitor->get('user_id'),
						'user_name' => $this->visitor->get('user_name'),
						'recieve_id' => $orders['buyer_id'],
						'recieve_name' => $orders['buyer_name'],
						'add_time' => gmtime(),
						'state' => 13,
						'jifen' => $order_integral['seller_has_integral'],
						'log_text' => '商家取消订单，返回用户扣除的积分',
					));

				}
				
				if($orders['status']==20)  //当定单为余额支付时，返回用户金额
				{
					
					$my_money_mod->edit('user_id='.$orders['buyer_id'],'money = money + '.$orders['order_amount']);
					
					//买家添加日志
					$buyer_log_text = '商家取消订单，钱返用户';
					$buyer_add_array=array(
						'user_id'=>$orders['buyer_id'],
						'user_name'=>$orders['buyer_name'],
						'order_id '=>$order_id,
						...
						'add_time'=>time(),
						'leixing'=>20,		
						'money_zs'=>"+".$orders['order_amount'],
						'money'=>$orders['order_amount'],
    ...
    function finished()
    {
        ...
        else
        {
			...
			$cat_info = $customer_mod->get('buyer_id='.$order_info['buyer_id'].' and store_id='.$this->visitor->get('user_id'));
			if(empty($cat_info))
			{
			...
			/* 交易完成，将订单积分表中的积分进行派发 by tyioocom */
			...
			if($order_integral)
			{
				...
				$integral['amount'] = $integral['amount']+$order_integral['seller_has_integral'];
								
				$ids =  $integral_mod->get('user_id = '.$this->visitor->get('user_id'));
				$idss =  $integral_mod->get('user_id = '.$order_info['buyer_id']);
				
				$id = $ids['id'];
				$integral_mod->edit($id,$integral);
				
				$integral = $integral_mod->get('user_id='.$order_info['buyer_id']);
				$integral['jifen'] = $integral['jifen'] + $order_integral['buyer_has_integral'];
				
				$integral_mod->edit($idss['id'],$integral);
			}
			/*派发结束 by tyioocom */
