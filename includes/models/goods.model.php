...
class GoodsModel extends BaseModel
{
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
		...
		$goods_intergral_mod = & m('goods_integral');
        $cg_table  = DB_PREFIX . 'category_goods';
		

        $fields = "g.goods_id, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image, g.taobao_url, g.paipai_url, " .
                "gs.spec_id, gs.spec_1, gs.spec_2, gs.color_rgb, gs.price, gs.stock, " .
                "s.store_name, s.region_id, s.region_name, s.credit_value, s.sgrade, s.cat_food, " .
                "gst.views, gst.sales, gst.comments,git.has_integral,git.max_exchange,git.bargin_price,git.integral_state";
        ...
        $tables = "{$this->table} g " .
				...
				"LEFT JOIN {$goods_intergral_mod->table} git ON g.goods_id=git.goods_id";
