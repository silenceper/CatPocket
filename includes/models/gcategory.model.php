...
class GcategoryModel extends BaseModel
{
	...
	function get_lists($parent_id = -1, $shown = false )
    {
        $conditions = "1 = 1";
        $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'";
        $shown && $conditions .= " AND if_show = 1 and show_index=0";
     ...
     */
    function get_gcategory_goods($cate_id, $num, $layer, $default_image = true)
    {
        $goods_list = array();
		...
		if($layer!='')
            ...
            /* 推荐商品 */
		$sql = "SELECT g.goods_id, g.goods_name, g.default_image, gs.price, gs.stock, gi.max_exchange, gi.bargin_price, gi.integral_state, gi.has_integral,gss.sales " .
			"FROM " . DB_PREFIX . "gcategory AS gy " .
			"   LEFT JOIN " . DB_PREFIX . "goods AS g ON gy.cate_id = g.cate_id_{$layer} " .
			...
			"   LEFT JOIN " . DB_PREFIX . "goods_statistics AS gss ON g.goods_id = gss.goods_id " .
			"WHERE " . $conditions . 
			"AND g.goods_id IS NOT NULL " .
			"ORDER BY gy.sort_order ";
