$(function(){
    ...
    $('*[ectype="dialog2"]').click(function(){
        ...
        /* 将选中的项通过GET方式提交给指定的URI */
        var uri = $(this).attr('uri');
		var url = uri + '&' + $(this).attr('name') + '=' + items;	
        var id = $(this).attr('dialog_id');
        var title = $(this).attr('dialog_title') ? $(this).attr('dialog_title') : '';
        var width = $(this).attr('dialog_width');
        ajax_form(id, title, url, width);
    ...
    $('span[ectype="order_by"]').click(function(){
        ...
        if(location.pathname.indexOf('/unlockmymkd/')>-1)
        {
                location.assign(SITE_URL + '/unlockmymkd/index.php?' + params.join('&'));
                return;
        }
