...
class Conf
{
    ...
    function load($conf)
    {
        $old_conf = isset($GLOBALS['CatPocket_CONFIG']) ? $GLOBALS['CatPocket_CONFIG'] : array();
        if (is_string($conf))
        {
            $conf = include($conf);
        }
        ...
        {
            $GLOBALS['CatPocket_CONFIG'] = array_merge($old_conf, $conf);
        }
        else
        {
            $GLOBALS['CatPocket_CONFIG'] = $conf;
        }
    }
    /**
     *    获取配置项
    ...
    function get($key = '')
    {
        $vkey = $key ? strtokey("{$key}", '$GLOBALS[\'CatPocket_CONFIG\']') : '$GLOBALS[\'CatPocket_CONFIG\']';
